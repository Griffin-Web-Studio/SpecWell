...
export default function WebsiteFrame(props) {
    ...
    const updateIframeHeight = useCallback( (height) => {
        setOptions((oldOption) => ({ ...oldOption, iframeHeight: height }));
    }, [setOptions]);

    const updateCommunicationStatus = useCallback( (status) => {
        setOptions((oldOption) => ({ ...oldOption, establishedCommunication: status }));
    }, [setOptions]);
